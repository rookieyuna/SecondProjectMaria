...
@WebServlet("/adminpage/ad_experience.do")
public class sub05Controller extends HttpServlet
{
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
	{
		...
		*/
		...
		//출력할 레코드 추출
		List<marketApplicationDTO> boardLists = dao.selectListExp(map);
		//자원 반납
		dao.close();
		
		//페이지 번호를 생성하기 위해 메서드 호출
		String pagingImg = BoardPage.pagingStr(totalCount, pageSize,
				blockPage, pageNum, "../adminpage/ad_experience.do");
		...
		//View로 포워드를 걸어준다.
		req.getRequestDispatcher("/adminpage/ad_experience.jsp").forward(req, resp);
	}
}
