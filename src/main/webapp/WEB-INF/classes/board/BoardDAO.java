...
// 게시판의 게시글DB들의 갯수 및 순번
   ...
   public List<BoardDTO> selectList(Map<String, Object> map, String cate) {
      ...
      try {
         ...
         query += " ( SELECT * FROM ( SELECT B.*, M.name, M.email FROM member M INNER JOIN board B ON M.id=B.id) board where category like '"+category+"' ) Cate ";

         if (map.get("searchWord") != null) {
            query += " ) SEARCH WHERE " + map.get("searchField") + " LIKE '%" + map.get("searchWord") + "%' ";
         }
   ...
   public BoardDTO getId(String id) {
      BoardDTO dto = new BoardDTO();
      String query = "SELECT * FROM member WHERE id LIKE '"+ id +"' ";
   ...
   public int insertWrite(BoardDTO dto) {
      int result = 0;
      try {
         String query = "INSERT INTO board (id, title, content, category) "
               + " VALUES (?, ?, ?, ?)";
   ...
   public int insertFile(BoardDTO dto) {
      ...
      try {
         ...
         String query = "INSERT INTO board ( "
               + "id, title, content, ofile, sfile,category) " 
               + " VALUES ( "
               + " ?, ?, ?, ?, ?, ?) ";
   ...
   public int pinsertWrite(BoardDTO dto) {
      ...
      try {
         String query = "INSERT INTO board ( " 
               + " id, title, content, postdate, category) "
               + " VALUES (?, ?, ?, ?, ?)";
   ...
   public Map<String, BoardDTO> calList(String data) { 
      ...
      String query =  "SELECT B.*, DATE_FORMAT(postdate, '%Y-%m-%d') pdate "
              + " FROM board B "
              + " WHERE category='proB' AND DATE_FORMAT(postdate, '%Y-%m')='"+data+"' " 
   ...
   public List<BoardDTO> mainViewtList(String cate, int a) {
      ...
      try {
    	 String query = "SELECT tb.* FROM (SELECT * FROM board WHERE category=?) tb order by postdate desc LIMIT 0, ?";
