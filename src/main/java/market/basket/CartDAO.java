...
public class CartDAO extends DBConnPool{
	...
	public int insertWrite(CartDTO dto) {
		...
		try { 
			String query = "INSERT INTO cart ( "
					+ " count_num, total_price, product_no, id"
			...
			psmt.setInt(3, dto.getProduct_no());
	 ...
	 */
	/*
	 * public List<MKProductDTO> selectPList() { List<MKProductDTO> board = new
	 * Vector<MKProductDTO>();
	 * 
	 * String query =
	 * "SELECT product.product_sfile, product.product_name, price, milage" +
	 * " FROM product INNER JOIN cart " +
	 * "ON product.product_no = cart.product_no;";
	 * 
	 * try { psmt = con.prepareStatement(query); rs = psmt.executeQuery();
	 * 
	 * while (rs.next()) { MKProductDTO dto = new MKProductDTO();
	 * 
	 * dto.setProduct_name(rs.getString(2)); dto.setPrice(rs.getInt(3));
	 * dto.setMilage(rs.getInt(4)); dto.setProduct_sfile(rs.getString(1));
	...
	public int deleteProduct(String to) { 
        ...
        try {
            ...
            psmt = con.prepareStatement(query); 
            psmt.setInt(1, Integer.parseInt(to));  
            //쿼리실행
            result = psmt.executeUpdate(); 
        } 
        catch (Exception e) {
